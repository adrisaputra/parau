									...
									<div class="card-content">
										...
										<div id="view-input-fields">
											<div class="row">
												...
												<input type="hidden" min="0" id="" name="selling_transaction_id" value="{{ $sellingTransaction->id }}" style="@if ($errors->has('product_id{{$item->id}}'))border-bottom: 2px solid #ff5252;@else color: black; @endif" readonly>
												<br><br>
												@php $total_belanja = 0; @endphp

												@foreach ($sellingDetail as $item)
												@php $total_belanja = $total_belanja + ($item->product->selling_price * $item->amount); @endphp
												<div class="col s12 confirm-cart" style="margin-bottom:3rem;" id="detail-{{$item->id}}">
													...
													<div class="col s4">
														...
														<div class="col s12 mb-5">
															<label for="selling_price">{{ __('Harga Beli Satuan (Rp.)') }}</label>
														...
														<div class="col s12 mb-5 p-0">
															...
															<div class="col s4">
																...
																@php $untung = ($item->product->selling_price - $item->product->selling_price) / $item->product->selling_price * 100; @endphp
													...
													<div class="col s4 mb-5 p-0">
														...
														<p style="font-size:40px;font-weight:bold;margin: 0 0 1rem 1rem;">Rp. <span id="sub-total-price-out">{{ number_format($item->product->selling_price * $item->amount, 2,',','.') }}</span></p>
														<input type="hidden" class="sub-total-price" id="sub-total-price" value="{{ $item->product->selling_price * $item->amount }}" style="@if ($errors->has('total-price{{$item->id}}'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
...
<script>
	...
	$('.confirm-cart input').on("input", function() {
		...
		// COUNT TOTAL PRICE
		var sub_total_price = amount * selling_price;
		...
		if($(this).attr('id') == 'selling_price'){
			profit = Math.round(((selling_price - selling_price) / selling_price * 100) * 100) / 100;
			$(detail_id + " #profit").val(profit);
		}
		else{
			selling_price = Math.round(selling_price * (profit/100 + 1) * 100) / 100;
