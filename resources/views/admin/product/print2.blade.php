...
<body>
  ...
  @for($i=0;$i<$jumlah;$i++) 
			@if($product[$i])
      <table style="height:50mm;width:70mm;margin-left:-80px;margin-top:-46px">
        <tr><th style="border-color: white"><center>
          <p style="color:black">{{ $product[$i]->product_name }}</p>
          <img src="data:image/png;base64,{{ DNS1D::getBarcodePNG($product[$i]->code, 'C128', 1.5, 70, [1, 1, 1], true) }}" alt="barcode" /> 
        </center></th>
        </tr>
      </table>
      @endif
