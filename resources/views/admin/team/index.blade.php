								...
								<div class="col s12 " style="overflow-x:auto;">
									...
									<tbody>
										...
										<tr>
											...
											<td>{{ $v->description }}</td>
											<td>
												<a class="mb-12 btn waves-effect waves-light blue darken-1 btn-small" href="{{ url('/worker/'.$v->id ) }}">Pekerja</a>
												@can('ubah-data')
									...
									<div class="float-right">{{ $team->appends(Request::only('search'))->links() }}</div>
								</div>
							</div>
							</div>
							@endcan
