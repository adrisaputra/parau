        ...
        <div class="col s12">
            <div class="container">
                                                ...
                                                <table class="highlight">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr role="row" class="odd">
                                                            ...
                                                            <td>{{ $row->selling_detail->count() }}</td>
                                                            <td>{{ number_format($row->total_price, 0, ',', '.') }}</td>
                                                            <td>{{ number_format($row->discount, 0, ',', '.') }}</td>
                                                            <td>{{ number_format($row->total_price-$row->discount, 0, ',', '.') }}</td>
                                                            <td>{{ $row->updated_at }}</td>
                                                            {{-- <td>{{ $row->created_at }}</td> --}}
                                                            <td class="p-1">
                                                                @can('ubah-data')
                                                                @endcan
                                                                <a class="btn mb-12 btn waves-effect waves-light blue darken-1 btn-small" href="{{ url('/' . Request::segment(1) . '/detail/' . $row->id) }}"><small class="nav-icon fas fa-list"></small> Detail</a>
                                                                {{-- <a class="btn btn-small waves-effect waves-light red darken-1" onclick="return confirm('Are you sure?')" href="{{ url('/' . Request::segment(1) . '/hapus/' . $row->id) }}"><small class="nav-icon fas fa-trash"></small> Hapus</a> --}}
                                                            </td>
                                                        </tr>
                                                        @endforeach
                                                    </tbody>
                ...
                <div style="bottom: 40px; right: 19px;" class="fixed-action-btn direction-top">
                    @can('tambah-data')
                    <a href="{{ url('/cashier') }}" class="btn-floating btn-large waves-effect waves-light green darken-2"><i class="material-icons">add</i></a>
                    @endcan
