                                            ...
                                            <div class="col s12 " style="overflow-x:auto;">
                                                <table class="highlight">
                                                    ...
                                                    <tbody>
                                                        @foreach ($outlet as $v)
                                                        <tr>
                                                            <td>{{ ($outlet->currentpage() - 1) * $outlet->perpage() + $loop->index + 1 }}</td>
                                                            <td>{{ $v->outlet_name }}</td>
                                                            <td>{{ $v->phone }}</td>
                                                            <td>{{ $v->address }}</td>
                                                            <td>
                                                                lat: {{ $v->lat }} <br>
                                                                long: {{ $v->long }} <br>
                                                                <a href="https://maps.google.com/?q={{ $v->lat }},{{ $v->long }}" target="_blank"><i class="fa fa-eye"></i> lihat peta</a>
                                                            </td>
                                                            <td>
                                                                @can('ubah-data')
                                                ...
                                                <div class="float-right">{{ $outlet->appends(Request::only('search'))->links() }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    @endcan
