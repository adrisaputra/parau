								...
								<form class="row">
									<p style="text-align:center;font-size:20px;font-weight:bold">Tambah {{ __($title) }}</p>
										...
										<div class="input-field col s6">
											...
											<input type="text" id="down_payment" name="down_payment" value="@if(old('down_payment')){{ old('down_payment') }}@else 0 @endif" onkeyup="formatRupiah(this, '.')" style="@if ($errors->has('down_payment'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
											<input type="hidden" id="down_payment" name="sum_payment" value="{{ ($project_detail->service_value - $project->discount) - $sum_payment }}">
										...
										<div class="input-field col s6">
											...
											<input type="text" id="date" onChange="getNumberOfDays()" class="datepicker" name="date" value="{{ date('d-m-Y') }}" style="@if ($errors->has('date'))border-bottom: 2px solid #ff5252;@else color: black; @endif" readonly>
