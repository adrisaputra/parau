												...
												<table>
													@foreach($v->project_detail as $x)
														<tr>
															...
															<td style="padding: 1px 5px;width:50%;border: 0px solid #f4f4f4;">: Tim {{ $x->team }}</td>
														</tr>
													@endforeach
												</table>
											</td>
											<td>
												<a class="btn mb-12 btn waves-effect waves-light blue darken-1 btn-small" href="{{ url('/progress-detail/' . $v->id) }}" style="padding-bottom:10px">Detail</a>
											</td>
										</tr>
										@endforeach
									</tbody>
									</table>
									<div class="float-right">{{ $project->appends(Request::only('search'))->links() }}</div>
								</div>
							</div>
							</div>
							@endcan
