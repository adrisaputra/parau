...
@section('konten')
    ...
    <div id="main">
      <div class="row">
		...
		<form action="{{ url('/'.Request::segment(1).'/edit/'.$group->id.'/'.$menu->id.'/'.$sub_menu_access->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
		{{ csrf_field() }}
        ...
        <div class="col s12">
          <div class="container">
								...
								<form class="row">
									...
									<div class="col s12">
										...
										<div class="input-field col s12">
											...
											<input type="text" id="menu_name" name="menu_name" value="{{ $menu->menu_name }}" disabled style="@if ($errors->has('group_name'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
										</div>

										<div class="input-field col s12">
											<select class="select2 browser-default" name="sub_menu_id">
												<option value="">- Pilih Menu -</option>
												@foreach($sub_menu as $v)
													<option value="{{ $v->id }}" @if($sub_menu_access->sub_menu_id==$v->id) selected @endif>{{ $v->sub_menu_name }}</option>
												@endforeach
											</select>
											@if ($errors->has('sub_menu_id'))<small><div class="error">{{ $errors->first('sub_menu_id') }}</div></small>@endif
										</div>
			...
			<div style="bottom: 90px; right: 19px;" class="fixed-action-btn direction-top">
				...
				<a href="{{ url('/'.Request::segment(1).'/'.$group->id.'/'.$menu->id) }}" class="btn-floating btn-large waves-effect waves-light red darken-2"><i class="material-icons">arrow_back</i></a>
			</div>
          </div>
		</form>
