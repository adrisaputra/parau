...
@section('konten')
    ...
    <div id="main">
      <div class="row">
	   ...
	   <form action="{{ url('/'.Request::segment(1).'/edit/'.$menu->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
	   {{ csrf_field() }}
								...
								<form class="row">
									...
									<div class="col s12">
										<div class="input-field col s12">
											...
											<input type="text" id="menu_name" name="menu_name" value="{{ $menu->menu_name }}" style="@if ($errors->has('menu_name'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
											@if ($errors->has('menu_name'))<small><div class="error">{{ $errors->first('menu_name') }}</div></small>@endif
										</div>
										<div class="input-field col s12">
											<label for="link">{{ __('Link') }}</label>
											<input type="text" id="link" name="link" value="{{ $menu->link }}" style="@if ($errors->has('link'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
											@if ($errors->has('link'))<small><div class="error">{{ $errors->first('link') }}</div></small>@endif
										</div>
										<div class="input-field col s12">
											<label for="attribute">{{ __('Atribute') }}</label>
											<input type="text" id="attribute" name="attribute" value="{{ $menu->attribute }}" style="@if ($errors->has('attribute'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
										</div>
										<div class="input-field col s12">
											<label for="position">{{ __('Posisi') }}</label>
											<input type="text" id="position" name="position" value="{{ $menu->position }}" style="@if ($errors->has('position'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
											@if ($errors->has('position'))<small><div class="error">{{ $errors->first('position') }}</div></small>@endif
										</div>
										<div class="input-field col s12">
											<label for="desc">{{ __('Deskripsi') }}</label>
											<input type="text" id="desc" name="desc" value="{{ $menu->desc }}" style="@if ($errors->has('desc'))border-bottom: 2px solid #ff5252;@else color: black; @endif">
